<template>
  ...
  >
      ...
      <div class="order-bg">
        <div class="left">
          ...
          <div
            v-if="isUnpaid"
            class="time"
          >{{payTimeFormat}} 后取消订单</div>
        </div>
        <img
          v-if="isUnpaid"
          class="right"
          src="../../assets/order/ic_clock.png"
          alt=""
        >
        <img
          v-else-if="isUnsend"
          class="right"
          src="../../assets/order/ic_box.png"
          alt=""
        >
        <img
          v-else-if="isUntake"
          class="right"
          src="../../assets/order/ic_truck.png"
          alt=""
        >
        <img
          v-else-if="isUnevaluation"
          class="right"
          src="../../assets/order/ic_comment.png"
          alt=""
        >
        <img
          v-else-if="isCancel"
          class="right"
          src="../../assets/order/ic_cancel.png"
          alt=""
        >
        <img
          v-else-if="isComplete"
          class="right"
          src="../../assets/order/ic_done.png"
          alt=""
        >
        <img
          v-else
          class="right"
          src="../../assets/order/ic_refund.png"
          alt=""
        >
      </div>
    </div>
    ...
    >
      <div
        v-if="shopList[0].orderLogisticsDetailList && shopList[0].orderLogisticsDetailList.length > 1"
        class="logistics"
      >
        <img
          class="car"
          src="../../assets/order/ic_logistics.png"
          alt=""
        >
        <div class="content">
          该订单已拆成{{shopList[0].orderLogisticsDetailList && shopList[0].orderLogisticsDetailList.length}}个包裹发出，点击查看具体物流详情
        </div>
      </div>
      <div
        v-else
    ...
    >
      <div
        @click="goStoreDetail(shop.storeId)"
        class="shop-header"
      >
    ...
    <div class="orderInfo">
      ...
      <div
        v-if="paymentAccount(this.shopList[0].latestStatus) && shopList[0].transStatementDetail"
        class="one-line"
      >
      ...
      <template v-if="shopList[0].orderTimeRefundDetail.length > 0">
        ...
        <div class="one-line">
          ...
          <div
            v-if="shopList[0].orderTimeRefundDetail.filter((item) => item.status === 7).length > 0"
            class="right"
          >{{shopList[0].orderTimeRefundDetail.filter((item) => item.status === 7)[0].desc}}</div>
          <div
            v-else-if="shopList[0].orderTimeRefundDetail.filter((item) => item.status === 8).length > 0"
            class="right"
          >{{shopList[0].orderTimeRefundDetail.filter((item) => item.status === 8)[0].desc}}</div>
        </div>
        <div class="one-line">
        ...
        <div
          v-if="shopList[0].orderTimeRefundDetail.filter((item) => item.status === 11).length > 0"
          class="one-line"
        >
    ...
    <div class="bottom-btn">
      <div
        v-if="showCancel(shopList[0].latestStatus)"
        class="cancel"
        @click="goCancel"
      >取消订单</div>
      <div
        v-if="showRefund(shopList[0].latestStatus)"
        class="pay-hui"
        @click="goRefund"
      >退款</div>
      <div
        v-if="shopList.length > 0 && shopList[0].isCashBack"
        class="back"
        @click="goBack"
      >分期返现</div>
      <div
        v-if="showPay(shopList[0].latestStatus)"
        class="pay"
        @click="goPay"
      >立即付款</div>
      <div
        v-if="showConfirm(shopList[0].latestStatus)"
        class="pay"
        @click="goConfirm"
      >确认收货</div>
      <div
        v-if="showEvaluation(shopList[0].latestStatus)"
        class="pay"
        @click="goEvaluation"
      >去评价</div>
      <div
      ...
      >去评价</div>
      <div
        v-if="showCancelRefund(shopList[0].latestStatus)"
        class="pay"
        @click="goCancelRefund"
      >取消退款</div>
    </div>
...
export default {
  ...
  computed: {
    ...
    showLogistics() {
      return this.shopList[0].orderLogisticsDetail &&
        this.shopList[0].orderLogisticsDetail.logisticsDto &&
        this.shopList[0].orderLogisticsDetail &&
        this.shopList[0].orderLogisticsDetail.logisticsDto.list.length > 0
      // const arr = [4, 5, 11]
